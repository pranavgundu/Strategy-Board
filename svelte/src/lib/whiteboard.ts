...
export class Whiteboard {
  ...
  private changeListeners: Array<(state: WhiteboardState) => void> = [];
  ...
  constructor(model: Model, options: WhiteboardOptions = {}) {
    this.model = model;
    ...
    if (options.bindLegacyDOM !== false) {
      this.bindLegacyDOM();
    }
  }
  ...
  public setColor(color: number): void {
    if (color < 0 || color > 4 || this.currentColor === color) return;
    this.currentColor = color;
    this.notifyChange();
  }

  public canUndo(): boolean {
    return this.getCurrentUndoHistory().length > 0;
  }
  ...
  private emitTo(
    listener: (state: WhiteboardState) => void,
